...
import (
	...
	"github.com/Fantom-foundation/go-lachesis/src/common/testing/ascii_generator"
...
func (p play) SelfParent() string {
	return p.selfParent
}

func (p play) OtherParent() string {
	return p.otherParent
}

func (p play) Index() int64 {
	return p.index
}
...
var _ ascii_generator.PlayGeneric = (*play)(nil)
...
func initConsensusPoset(db bool, t testing.TB) (*Poset, map[string]EventHash) {
	...
	// generate dot and ascii file based on play array
	playsInterface := make([]interface{}, len(plays))
	for i, p := range plays {
		playsInterface[i] = p
	}
	ascii_generator.ConvertPlaysToAscii(playsInterface, "initConsensusPoset")
